...
> 근거 문서: `cpp-code-conventions.md` (2026-08-13 결정: **Google C++ Style Guide + C++ Core Guidelines 기반**)  
...
- 예외: 값을 돌려주기만 하는 저렴한 접근자는 snake_case(`fd()`, `active()`, `size()`), 동작을 수행하는 함수는 UpperCamelCase(`Connect()`, `Commit()`)
...
## 3. 언어 기능 규칙 (서버 코드 = `ejd_lib`, `ejd_server`, 봇 = `ejd_bot`)
...
- **적용 범위**: 위 엄격 옵션은 서버 코드에만 적용. `ejd_tests`에는
...
- **include 가드: `#pragma once`** (Google은 define 가드지만,
  주요 컴파일러 전부 지원 + 오타/복붙 실수 원천 차단을 이유로 절충)
...
| `ejd_tests` | 실행파일 | `tests/*.cc` — lib + GTest 링크 (`-Wall -Wextra -Werror`만, `-fno-exceptions`/`-fno-rtti` 제외) |
| `ejd_bot` | 실행파일 | `botclient/main.cc` — lib 링크 (엄격 옵션 적용) |
...
제출 시점 태그: `git tag vX.Y.Z-submit` (`v0.1.0-submit`: 1차 제출본, `v0.1.2-submit`: 2차 제출본)
